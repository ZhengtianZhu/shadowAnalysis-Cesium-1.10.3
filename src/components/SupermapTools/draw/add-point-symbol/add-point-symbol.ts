...
import tool from '@/js/tool/tool'        //提示等工具
import Resource from '@/js/local/lang'  //语言资源
import { initHandler, handlerDrawing, clearHandlerDrawing } from "@/js/common/drawHandler"    //公共handler.js
import createTooltip from '@/js/tool/tooltip2'
import s3mModels from "./models"
import PolygonEmitter from "./polygon-emitter"
import type { PointSymbolProps, PointSymbolState } from "./typing"
import { useViewer } from "@/js/store"

function addPointSymbol (props: PointSymbolProps) {
    let $v = useViewer()
    let viewer = $v.viewer
    // 设置默认值数据
    let state = reactive<PointSymbolState>({
