...
import tool from '@/js/tool/tool.js'        //提示等工具
       //提示等工具
//1.I might misunderstand the meaning of these resource
//import resource from '../../../js/local/lang.ts'  //语言资源
...
//Zhengtian added this sentence
interface clickEvent {//here we have clickEvent
    keyPath: Array<string>
}
const handleClick = (e:clickEvent) => {}  //Besides, there exists clickEvent here
//Zhengtian added this sentence
/* interface   viewer  {//here we have clickEvent
...
// setEntity(cesiumAir);
    ...
    let state = reactive({
        ...
        //先运行原生的，看看是干什么的；这样才能有针对性地找个平替，去实现功能
        // there seems there exists no layerShadowType 
        //I want to denote it.
        layerShadowType: 255,// Cesium.ShadowType.ALL, //  I don't know the value图层上阴影类型：所有的模型都产生阴影。
    ...
    if (props) {
        for (let key in props) {
            if (state.hasOwnProperty(key)) {//法会返回一个布尔值，指示对象自身属性中是否具有指定的属性（也就是，是否有指定的键）
                state[key] = props[key]
            } else {
    ...
    let color = ['#0000ff'], gamma: any;
    color = color.concat(gradientColors('#0000ff', '#00ffff', 20,gamma))
        .concat(gradientColors('#00ffff', '#00ff00', 20,gamma))
        .concat(gradientColors('#00ff00', '#ffff00', 20,gamma))
        .concat(gradientColors('#ffff00', '#ff7f00', 20,gamma))
        .concat(gradientColors('#ff7f00', '#ff0000', 20,gamma));
    

    // console.log(    "hello"+window.viewer._element  );
    if (storeState.isViewer) {
        
        if (!window.tooltip) {
            window.tooltip = createTooltip(window.viewer._element);
        }
        init()
    }
    ...
    function init() {   //初始化函数
        ...
        window.viewer.shadows = state.shadowShow;
        // viewer.terrainShadows = state.terrainShadows;
        // layers = window.viewer.scene.layers.layerQueue;
        scene.globe.enableLighting = true;
        scene.skyBox.show = true;
        window.viewer.clock.multiplier = 1;
        window.viewer.clock.shouldAnimate = true;
        
        //I want to use a function to rewrite this?
        shadowQuery = new Cesium.ShadowQueryPoints(scene);    //创建阴影查询对象

        //Zhengtian annotated them
        // s3mInstanceCollection = new Cesium.S3MInstanceCollection(scene._context);//实例化对象
    ...
    */
    ...
    function timeChanging(arr) {
        ...
        window.viewer.clock.currentTime = Cesium.JulianDate.fromDate(tm);
        currentTimeArr = arr;
    };
    ...
    function setCurrentTime(tm) {
        tm=parseInt(tm);
        let h = tm / 4;// Zhengtian has rewritten the code here
        let m = (tm % 4) * 15;
    ...
    function LEFT_CLICK(e) {
        if (state.shadowBodyShow) {
            let box = window.scene.pick(e.message.position);
            if (box && box.id) {
            ...
            let position1 = window.scene.pickPosition(e.message.position);
    ...
    function clear() {
        ...
        window.tooltip.setVisible(false);
        closeBubble();
        window.viewer.eventManager.removeEventListener("CLICK", LEFT_CLICK); //移除鼠标点击事件监听
        shadowQuery.clear();
    ...
    function getShadowRadioColor(shadowRadio) {
        shadowRadio=parseInt(shadowRadio);
        let index = (shadowRadio * 100);
        let col = color[index];
